...
Because the theine command will wait for a server to become available if it's
not, it will work even if the theine server is started after the client. You
will need to use theine_current_ruby because Foreman uses bundle exec and
theine won't work if you previously used theine_set_ruby.
So basically:

  theine_server: bundle exec theine_server
  server: theine_current_ruby rails server
