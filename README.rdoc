...
I put this into my `~/.rvm/hooks/after_use_jruby_custom' and chmoded it a+x. It makes JRuby start up noticably faster for Rails projects.
...
So in other words, the options to pass to jruby are:

  -Xcompile.invokedynamic=false -J-XX:+TieredCompilation -J-XX:TieredStopAtLevel=1 -J-noverify -Xcompile.mode=OFF

== Using with Foreman

At the moment you can only use Theine with Foreman if you do not set the client to
use Ruby MRI (theine_set_ruby command). So you should not do that.

Other than that it's pretty easy. Because the theine command will wait for a
server to become available if it's not, it will work even if the theine server is
started after the client. So basically:

  theine_server: bundle exec theine_server
  server: theine rails server
